...
AJS.bind("init.rte", function() {
    ...
    function receiveMessage(event)
    {
        if (event.origin !== domain){
            ...
            //override selection when error occurs
            if(selection.tagName!="P"){
                var my_html = selection.outerHTML;
                my_html = my_html.substring(my_html.indexOf("<img class=", 1), my_html.length);
                selection = document.createElement("p");
                selection.innerHTML = my_html;
            }
